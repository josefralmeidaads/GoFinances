...
class ImportTransactionsService {
  async execute(filePath: string): Promise<Transaction[]> {
    ...
    const existentCategoriesTitles = existentsCategories.map( cat => cat.title);

    const addCategoriesTitles = categories.filter( categories => !existentCategoriesTitles.includes(categories)).
